...
typedef void (*t_cdc_callback)(char);
t_cdc_callback cdc_cbk;

static void cdc_rxonly (usbd_device *dev, uint8_t event, uint8_t ep) {
    int32_t _t, i;
    _t = usbd_ep_read(dev, CDC_RXD_EP, fifo, CDC_DATA_SZ);
    for (i = 0; i < _t; i++) {
        cdc_cbk(fifo[i]);
        //printf("cdc_rxonly %c\n", fifo[i]);
    }
...
extern void cdc_init_rcc (void);

void cdc_main_irq(t_cdc_callback cbk) {
    cdc_cbk = cbk;
    cdc_init_rcc();
    cdc_init_usbd();
...
void cdc_write_data(const char *buf, size_t len) {
    int n = usbd_ep_write(&udev, CDC_TXD_EP, buf, len);
}