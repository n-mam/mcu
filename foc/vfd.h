...
struct vf_controller_t {
    ...
    static void pwm_trampoline(timer_event_t event) {
        if (active) active->pwm_callback(event);
    }

    // TIM2 encoder callback trampoline
    static void encoder_trampoline(timer_event_t event) {
        if (active) active->encoder_callback(event);
    }
    ...
    void start() {
        ...
        hw.initialize_phase_pwm_timer(&pwm_trampoline);
...
vf_controller_t *vf_controller_t::active = nullptr;
