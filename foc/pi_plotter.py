...
pattern = re.compile(
    ...
    r"vd_i:(?P<d_i>[-+0-9.eE]+)\s+"
    r"vq_i:(?P<q_i>[-+0-9.eE]+)\s+"
    r"s_i:(?P<s_i>[-+0-9.eE]+)"
)
# ============================================================
# EMA FILTER
...
def apply_axis_ranges():
    ...
    try:
        new_d_min = float(d_min_entry.get())
        new_d_max = float(d_max_entry.get())

        new_q_min = float(q_min_entry.get())
        new_q_max = float(q_max_entry.get())

        new_s_min = float(s_min_entry.get())
        new_s_max = float(s_max_entry.get())
    ...
    if new_s_min >= new_s_max:
        status_label.config(
            text="Speed min must be smaller than Speed max",
            fg="red"
        )
        return
...
def default_axis_ranges():
    ...
    d_min_entry.insert(0, str(DEFAULT_D_MIN))
    d_max_entry.delete(0, tk.END)
    d_max_entry.insert(0, str(DEFAULT_D_MAX))
    # Q
    q_min_entry.delete(0, tk.END)
    q_min_entry.insert(0, str(DEFAULT_Q_MIN))
    q_max_entry.delete(0, tk.END)
    q_max_entry.insert(0, str(DEFAULT_Q_MAX))
    # Speed
    s_min_entry.delete(0, tk.END)
    s_min_entry.insert(0, str(DEFAULT_S_MIN))
    s_max_entry.delete(0, tk.END)
    s_max_entry.insert(0, str(DEFAULT_S_MAX))
    # Apply D
    ax_d.set_ylim(
        d_min,
        d_max
